...
typedef struct mscclppDeviceAllGatherHandle {
  char* slab;
  char* control;
  char* peerSlabs[MSCCLPP_DEVICE_ALLGATHER_MAX_RANKS];
  unsigned long long* peerReady[MSCCLPP_DEVICE_ALLGATHER_MAX_RANKS];
  unsigned long long* peerDone[MSCCLPP_DEVICE_ALLGATHER_MAX_RANKS];
  size_t peerReadySlotStride[MSCCLPP_DEVICE_ALLGATHER_MAX_RANKS];
  size_t peerDoneSlotStride[MSCCLPP_DEVICE_ALLGATHER_MAX_RANKS];
...
// same maxBytesPerRank. One node and balanced two-node communicators are
// supported; two-node communicators automatically use host-staged RDMA.
ncclResult_t mscclppGetDeviceAllGatherHandle(
    ncclComm_t comm, size_t maxBytesPerRank,
    mscclppDeviceAllGatherHandle_t* handle);

// Backend-selecting variant. CUDA IPC requires peer access between every pair
// of participating GPUs. The legacy entry point above selects host memory.
...
static __device__ __forceinline__ char* mscclppDeviceCollectiveSlab(
    const mscclppDeviceAllGatherHandle_t& h, int slot, int rank) {
  if (h.backend == mscclppDeviceCollectiveCudaIpc ||
      h.backend == mscclppDeviceCollectiveHostRdma) {
...
static __device__ __forceinline__ bool mscclppDeviceCollectiveHandleValid(
    const mscclppDeviceAllGatherHandle_t& h) {
...
static __device__ __forceinline__ int liteAllGatherBlock(
  ...
  if (tid == 0) {
    status = mscclppDeviceCollectiveSuccess;
    if (!mscclppDeviceCollectiveHandleValid(h) || srcVoid == nullptr ||
        dstVoid == nullptr || bytesPerRank == 0 ||
  ...
  if (tid == 0) {
    ...
    if (h.backend == mscclppDeviceCollectiveHostRdma) {
      h.publishedBytes[static_cast<size_t>(slot) *
                           h.publishedBytesSlotStride +
                       h.rank % (h.nranks / 2)] = bytesPerRank;
      __threadfence_system();
    }
    *mscclppDeviceAllGatherReady(h, slot, h.rank) = epoch;
    __threadfence_system();
  ...
  for (size_t i = tid; i < totalBytes; i += threadCount) {
    ...
    dst[i] = mscclppDeviceCollectiveSlab(h, slot, sourceRank)[sourceOffset];
  }
  __syncthreads();
