...
inline constexpr bool IsHostMemory = Kind == MemoryType::Host || 
                                     Kind == MemoryType::HostPinned || 
...
class CopyPrimitives {
 public:
  /*
  T is the writable destination element type. 
  SrcT is deduced separately so the source can be either T or const T.
  */
  template <MemoryType Src, MemoryType Dst, typename SrcT>
  Completion copy(Span<SrcT> source, 
                  Span<T> destination,
  ...
  /*
  Submit a copy without allocating a completion event:
    Host-to-host copy finished before returning; 
    CUDA copy is only enqueued on the given stream.
  */
  template <MemoryType Src, MemoryType Dst, typename SrcT>
  void enqueueCopy(Span<SrcT> source, 
                   Span<T> destination,
                   cudaStream_t stream = nullptr) const {
    // SrcT can be either T or 'const T'
    ...
    if constexpr (copy_detail::IsHostMemory<Src> &&
      ...
      throwCudaError(cudaMemcpyAsync(destination.data, 
                                     source.data, 
                                     bytes,
  ...
  template <MemoryType Src, MemoryType Dst, typename SrcT>
  Completion copyRows(Rows<SrcT> source, 
                      Rows<T> destination,
                      cudaStream_t stream = nullptr) const {
  ...
  template <MemoryType Src, MemoryType Dst, typename SrcT>
  void enqueueCopyRows(Rows<SrcT> source, 
                       Rows<T> destination,
                       cudaStream_t stream = nullptr) const {
    // SrcT can be either T or 'const T'
    static_assert(std::is_same_v<std::remove_const_t<SrcT>, T>,
    ...
    if constexpr (copy_detail::IsHostMemory<Src> &&
      ...
      throwCudaError(
          cudaMemcpy2DAsync(destination.data, 
                            destination.stride * sizeof(T), 
                            source.data, 
