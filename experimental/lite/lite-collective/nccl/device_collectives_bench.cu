...
int main(int argc, char** argv) {
  ...
  if (localSize != nranks || nranks < 2 || nranks > 8) {
    if (rank == 0)
      std::fprintf(stderr, "benchmark requires 2-8 ranks on one node\n");
    MPI_Abort(MPI_COMM_WORLD, 1);
  }
  ...
  mscclppDeviceAllGatherHandle_t handle{};
  NCCL_CHECK(mscclppGetDeviceAllGatherHandle(comm, maxStagedBytes, &handle));

  if (rank == 0) {
    std::printf("GPU-driven device collective kernel latency, ranks=%d, "
